import philip from "./images/philip.png";
import erik from "./images/erik.png";
import hazem from "./images/hazem.jpg";
import elin from "./images/elin.png";
import isaaq from "./images/isaaq.jpg";

export const team = [
  {
    name: "Philip Risberg",
    image: `${philip}`,
    about: `I am a student at Medieinstitutet in Gothenburg where I am studying to become a Front End Developer. My long term goal is to make a living doing what I love, which is to be creative. I am very motivated by challenging myself and taking on responsibilities, which is in large part why I moved to Gothenburg from Scania for this education.
    This far into my education I have aquired the basics in Html, CSS3 and Javascript. I have worked with React, Typescript, Express, CSS-in-JS, MUI and more. I have also completed a course on UX design. What I would love now is some hands-on experience in the actual working life of a web developer.
    As a person I am calm, curious and friendly. My interest lies in anything tech related, creative, challenging and adventurous. Examples include strength training, drawing and some amateur level photography.
    If you would like to find out more about me or get in contact all relevant links are below.`,
    github: "https://github.com/Prisberg",
    linkedin: "https://www.linkedin.com/in/philip-risberg/",
  },

  {
    name: "Elin Arntén",
    image: `${elin}`,
    about: `Hello my name is Elin Arntén and I am Front End developer student,
with a background in testing. I was a part of Elon Musks spaceX testing. Now I'm back on the ground and because the space gave me anxiety. On my sparetime I like to go hiking in beautiful nature environment and I also like to crochet.`,
    github: "https://github.com/elinarnten",
    linkedin: "https://www.linkedin.com/in/elin-arnten/",
  },

  {
    name: "Hazem Kawas",
    image: `${hazem}`,
    about: `A front-end developer and UX designer. `,
    github: "https://github.com/hazem-89",
    linkedin: "https://www.linkedin.com/in/hazem-kawas-66971b185/",
  },

  {
    name: "Erik Matsegård",
    image: `${erik}`,
    about: `My name is Erik Matsegård, I'm a Senior Web Developer at Phiee. I have 10+ years experience in graphic design, UX/UI design & Front-end development.  
    When I'm not writing code or designing a new website i like to spend time with my other passion in life, which is exploring the world on my café racer.`,
    github: "https://github.com/matsegard",
    linkedin: "https://www.linkedin.com/in/erik-matseg%C3%A5rd-716a11220/",
  },

  {
    name: "Isaaq Guleed",
    image: `${isaaq}`,
    about: `Ex google ex facebook ex spotify entreprenaur, self-made millionaire. 10 years experience in the full-stack business.`,
    github: "https://rchtect.github.io",
    linkedin: "https://se.linkedin.com/in/isaaq-guleed-6a0614239",
  },
];
